...
export class LocatorsView extends DisposableBase implements vscodeTypes.WebviewViewProvider {
  ...
  constructor(vscode: vscodeTypes.VSCode, settingsModel: SettingsModel, browserServer: BrowserServer, extensionUri: vscodeTypes.Uri) {
    super();
    this._vscode = vscode;
    ...
    this._disposables = [
      ...
      this._browserServer.onInspectRequested(async ({ locator, ariaSnapshot, backendVersion }) => {
  ...
  public resolveWebviewView(webviewView: vscodeTypes.WebviewView, context: vscodeTypes.WebviewViewResolveContext, token: vscodeTypes.CancellationToken) {
    ...
    this._disposables.push(webviewView.webview.onDidReceiveMessage(data => {
      if (data.method === 'execute') {
        ...
        this._browserServer.highlight(this._locator.locator).then(() => {
        ...
        this._browserServer.highlightAria(this._ariaSnapshot.yaml).then(() => {
