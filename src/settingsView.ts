...
export class SettingsView extends DisposableBase implements vscodeTypes.WebviewViewProvider {
  ...
  constructor(vscode: vscodeTypes.VSCode, settingsModel: SettingsModel, models: TestModelCollection, reusedBrowser: ReusedBrowser, mcpConnection: McpConnection, extensionUri: vscodeTypes.Uri) {
    super();
    this._vscode = vscode;
  ...
  private _updateBrowsers() {
    ...
    for (const browser of this._reusedBrowser.state().browsers) {
      const pages = browser.contexts.flatMap(c => c.pages);
      const hostname = pages[0] ? new URL(pages[0].url).hostname : '';

      let svg = chromiumLogo;
      if (browser.name === 'chromium' && browser.channel?.startsWith('chrome'))
        svg = chromeLogo;
      else if (browser.name === 'chromium' && browser.channel?.startsWith('msedge'))
        svg = edgeLogo;
      ...
      browsers.push({
        svg,
        text: hostname || browser.channel || browser.name,
        title: [browser.name, browser.channel, hostname].filter(Boolean).join(' - '),
        actions: [
          {
            svg: mcpLogo,
            title: 'Connect MCP to this Browser',
            command: 'pw.extension.command.connectMCP',
            args: [browser.assistantMode ? undefined : browser.guid],
            disabled: this._mcpConnection.isConnectedTo(browser.guid),
          }
        ]
      });
    }
...
function htmlForWebview(vscode: vscodeTypes.VSCode, extensionUri: vscodeTypes.Uri, webview: vscodeTypes.Webview) {
  ...
  return html`<!DOCTYPE html>
    ...
    <body class="settings-view">
      ...
      <section id="browsers" class="vbox" aria-label="Browsers"></section>
...
const closeBrowsersAction = (vscode: vscodeTypes.VSCode, reusedBrowser: ReusedBrowser) => {
  return {
...
const webkitLogo = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 510 552" version="1.1">
    <defs>
